...
class sigesp_mis_c_contabiliza
{
	...
	function uf_select_activos_contabilizar($as_mes, $as_ano, $ai_estatus, &$ao_object, &$ai_totrows)
	{
		...
		if(!empty($as_mes))
		{
			if($as_mes!="00")
			{
				$ls_criterio=$ls_criterio." AND date_part('month',saf_depreciacion.fecdep) = '".$as_mes."' ";
			}
		}
		if(!empty($as_ano))
		{
			$ls_criterio=$ls_criterio." AND date_part('year',saf_depreciacion.fecdep) = '".$as_ano."'";
		}
		$ls_sql="SELECT saf_depreciacion.mondepmen AS monto, date_part('year',saf_depreciacion.fecdep) AS anio, ".
				"		date_part('month',saf_depreciacion.fecdep) AS mes, saf_dta.estact,  saf_dta.fecdesact,  ".
